...
- **ドラッグ＆ドロップ**: エクスプローラーからファイルをウィンドウに直接ドラッグ＆ドロップして開くこともできます。
...
パネル内の各タブを使って見た目を調整します。数値入力や色の設定には、直感的でモダンな専用コントロールを採用しています。

- **数値と単位の設定**:
  - フォントサイズや余白など、数値を扱う項目では、右側のドロップダウンから単位（px, em, %）を個別に選択できます。
  - **自動変換**: 単位を切り替えると、見た目の大きさを維持するように数値が自動的に再計算されます。
...
- **色を変える**:
  - 左側に現在の色のプレビュー、中央にカラーコードが表示されます。
  - マウスを近づけると右側に現れるパレットアイコンをクリックして、カラーピッカーから色を選択できます。
...
- **ショートカットによる装飾**: 以下のショートカットで素早く装飾を行えます（選択範囲がある場合はその範囲を囲みます）。
  - **太字**: `Ctrl + B`
  - **斜体**: `Ctrl + I`
  - **取り消し線**: `Ctrl + Shift + X`
  - **インラインコード**: `Ctrl + Shift + C`
  - **タスクリストのトグル**: `Ctrl + L` （現在の行をタスクリスト化、または完了/未完了を切り替えます）
