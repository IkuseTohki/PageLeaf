...
- **GUI CSS エディター**: プログラミングの知識がなくても、フォント、色、余白、段落スタイル（行間・字下げ等）を直感的にカスタマイズ。数値と単位、操作ボタンがコンパクトに統合された UI を採用。
...
- **チートシート機能**: Markdown 記法やショートカットキーをいつでも確認できるクイックリファレンス・ウィンドウを搭載。
- **フロントマター制御**:
  - Markdown の先頭に YAML 形式で設定を記述可能。
  - 新規作成時にタイトルや作成日時、カスタムプロパティを自動挿入する設定も完備。
...
PageLeaf は**ポータブル**な設計となっており、設定ファイル（`settings.yaml`）は**実行ファイル（.exe）と同じディレクトリ**に保存されます。
これにより、アプリケーションフォルダを移動するだけで、設定や作成した CSS スタイルをそのまま別の PC へ持ち運ぶことができます。

## ⌨️ ショートカットキー

| ショートカット        | 機能                              |
| --------------------- | --------------------------------- |
| `Shift + Alt + ← / →` | 編集モード / 閲覧モードの切り替え |
| `Ctrl + S`            | 保存                              |
