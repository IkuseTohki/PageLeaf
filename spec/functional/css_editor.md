...
- **マーカー設定**:
  ...
  - **ピリオドの有無 (ol)**: 番号の後にピリオド（.）を表示するかどうかの切り替え。
...
- **デザインプリセット**: アプリが提供する `extensions.css` に定義された 4 つの基本スタイルを選択可能。選択されたプリセットはフロントマターの `quote_style` に保存される。
  ...
  - **Modern (モダン)**: グラデーションライン（アイコンなし時）や洗練されたスタイル (`bq-modern`)。
- **アイコン制御**:
  - 引用符アイコン（“）の表示/非表示を切り替え可能 (`has-icon` / `no-icon` クラスの付与)。
- **引用元 (Citation)**:
  - 独自拡張 `[cite: ...]` で記述された箇所は `<cite>` タグに変換される。
- **詳細カスタマイズ (上書き)**:
  - ユーザーCSSファイルには、プリセットのプロパティを上書きするセレクタを記述する。
  - 例: `.bq-simple { border-left-color: red; }`
  - パネル上の各項目（文字色、背景色、枠線等）の編集内容は、このユーザーCSSに保存される。
...
- **初期ロード**: CSS ファイルを `AngleSharp.Css` ライブラリで解析し、各要素のプロパティを GUI の各項目にマッピングする。
...
- **リロードなしの同期 (Hot-Swapping)**: CSS ファイルの切り替え時、プレビュー画面全体をリロードするのではなく、HTML 内の `<link>` タグの参照先 (`href`) を JavaScript で動的に書き換えることで、スクロール位置を維持したまま瞬時にスタイルを反映させる。
