...
| `Ctrl + Shift + Q` | **引用元の挿入**         | 選択範囲を `[cite: ]` で囲む（または挿入）。カーソルは `]` の手前へ移動。 |
...
Markdown 内に `[cite: 引用元テキスト]` と記述することで、HTML レンダリング時に `<cite>引用元テキスト</cite>` へ変換する。
これは通常、`<blockquote>` 内の末尾に配置されることを想定している。
