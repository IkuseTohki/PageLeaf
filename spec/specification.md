...
- **モード切り替え**: 以下の 2 つのモードを切り替えることができる。
...
- **YAML フロントマター対応**: Markdown ファイルの先頭にある YAML 形式のメタデータブロックを認識する。
...
### 2.4. (将来的な拡張候補) エクスポート機能

※将来的な拡張候補として残すが、現在はメニューから削除済み。

### 2.5. (将来的な拡張候補) フロントマター活用

以下の機能は、フロントマターの活用アイデアとして検討中。

- **CSS (テーマ) のファイル単位指定**: フロントマターに `css: "dark-mode.css"` のように記述することで、ドキュメント単位でプレビュー用 CSS を指定可能にする。
- **クラス/ID の埋め込み**: `class: "report-style"` の記述により、プレビューのラッパー要素にクラスを付与し、スタイル切り替えを容易にする。
- **タイトルの同期**: フロントマターの `title` フィールドを、ウィンドウタイトルやタブ名と同期させる。
- **エクスポート時のメタデータ活用**: HTML/PDF 出力時に、`author` や `date` をフッターに挿入したり、HTML メタタグに反映させる。
...
- **CSS 切り替え**:
  - ユーザーはプルダウンメニューから適用する CSS スタイルを選択し、メイン表示・編集エリア（ビューアーモード時）のスタイルを動的に変更できる。
  - アプリケーション起動時に、特定のディレクトリ（例: `css/`）から利用可能な CSS ファイルを読み込み、プルダウンメニューにリスト表示する。
