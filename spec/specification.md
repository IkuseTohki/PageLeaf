...
- **入力支援機能**:
  ...
  - **脚注（注釈）挿入**: `Ctrl+Alt+F` でカーソル位置にポップアップを表示し、本文へのマーカー挿入と文末への定義追加を一括で行う。
...
- **脚注設定**: 脚注の参照マーカー（色、太字、括弧）、注釈エリア全体（文字サイズ、色、区切り線）、リストアイテム（行間、戻るリンク）のスタイルを編集できる。
- **フォントサイズ・数値指定**:
...
### 2.6. モーダルインタラクション (UI/UX)

設定画面やチートシートなどのモーダルウィンドウ表示時、ユーザーの集中を誘導し誤操作を防ぐための以下の振る舞いを実装する。

- **シャドウ・オーバーレイ**: メインウィンドウ全体を半透明の黒色レイヤーで覆い、背面の操作をブロックする。
- **オーバーレイの挙動**:
  - 表示・非表示時はフェードアニメーションを行う。
  - オーバーレイ部分（ウィンドウ外）をクリックすると、モーダルウィンドウを閉じる。
- **フォーカストラップ**: モーダル表示中は、Tabキー等によるフォーカス移動をモーダルウィンドウ内に限定し、背面へのフォーカス流出を防ぐ。
- **出現アニメーション**: モーダルウィンドウ表示時、フェードインに加え、スケールやスライド等の動きを伴って出現させる。
