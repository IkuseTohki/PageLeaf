...
- **目次 (TOC) 機能**: 編集中の Markdown から見出し（H1〜H3）を抽出し、目次リストを表示する。
...
- **CSS 管理**: 作成した CSS の保存、読み込み、削除、および適用する CSS の選択ができる。これらの機能は CSS 編集パネルの上部に集約される。
...
- **CSS 編集パネル**: 画面右側に配置。上部に CSS 選択ドロップダウンと保存・リセットボタンを備える。
